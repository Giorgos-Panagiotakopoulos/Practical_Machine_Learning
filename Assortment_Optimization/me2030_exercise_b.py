...
# Ο χρήστης πληκτρολογεί τους κωδικούς των προϊόντων για τους οποίους θέλει να κάνει πρόβλεψη εσόδων.
# Έλεγχος των κωδικών των προϊόντων, ώστε να βρίσκονται μέσα στο επιτρεπτό εύρος τιμών.
readAgain = True
while readAgain != False:
    readAgain = False
    productNumbers = input(
        'Set assortment typing product codes (0...' + str(TotalProducts - 1) + ') separated with space (e.g. 2 5 8): ')
    # Δημιουργία λίστας με τους κωδικούς που πληκτρολόγησε ο χρήστης
    assortment = productNumbers.split()
    # Έλεγχος κάθε κωδικού, ώστε να βρίσκεται σε επιτρεπτή περιοχή τιμών
    for item in assortment:
        if (int(item) < 0) or (int(item) > TotalProducts - 1):
            readAgain = True

# Από το dataframe των συνολικών δεδομένων, αφαιρούνται όλες οι στήλες που ανήκουν στο καλάθι που εξετάζουμε
X = baskets.drop(assortment, axis=1)
y = baskets[assortment]

# Αθροίζονται τα έσοδα όλων των προϊόντων που ανήκουν στο καλάθι που δημιούργησε ο χρήστης,
# ώστε να δημιουργηθεί ένας μονδιάστατος πίνακας y, ο οποίος μπορεί να χρησιμοποιηθεί στο μοντέλο
y = y.sum(axis=1)
# train test spit
...
info = []
info = y[:len(predictions_results['AdaBoostRegressor'])].tolist()
...
print('REVENUE FORECAST FOR PRODUCTS: ' + str(assortment))
